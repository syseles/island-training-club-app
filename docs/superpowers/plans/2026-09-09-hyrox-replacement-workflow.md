...
Add a v19 fixture with an existing booking and assert the current v22 state preserves all existing data while adding only `replacementUserId`, `replacementConfirmedAt`, and `replacementConfirmedBy` as null plus empty replacement collections.
...
Keep the current `STATE_VERSION` at 22: the additive replacement normalization runs in the existing migration seam and initializes replacement collections/fields without disturbing the attendance and reminder migrations already present on `testing`. Add the fields to every booking constructor. Reject reserved, attended, cancelled, expired, deferred, RSVP/free, started, and non-owner-ineligible records. Permit confirmed paid HYROX bookings, including pooled bookings with `sessionId === null`, until the calculated expiry.
