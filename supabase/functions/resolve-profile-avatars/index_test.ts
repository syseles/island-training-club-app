...
Deno.test('session attendee IDs use confirmed replacements and retain booking order', () => {
  assertEquals(
    effectiveAttendeeProfileIds([
      { profile_id: VIEWER_ID, replacement_profile_id: MEMBER_ID },
      { profile_id: HIDDEN_ID, replacement_profile_id: null },
    ]),
    [MEMBER_ID, HIDDEN_ID],
  );
});
