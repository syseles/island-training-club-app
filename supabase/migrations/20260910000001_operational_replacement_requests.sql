...
-- Confirmed replacements change the operational attendee only. Payment,
-- receipt, booking authorization, and payer history still use b.profile_id.
...
begin
  ...
  if not coalesce(v_is_paid, false) then return; end if;
  ...
  select coalesce(
    ...
    join public.profiles p on p.id = coalesce(b.replacement_profile_id, b.profile_id)
