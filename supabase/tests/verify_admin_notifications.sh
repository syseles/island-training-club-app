...
psql_bin="${ITC_NOTIFICATION_PSQL_BIN:-psql}"
...
psql_cmd=("$psql_bin" "${ITC_NOTIFICATION_TEST_DATABASE_URL}" -X -v ON_ERROR_STOP=1)

# Refuse to run against anything except an unused Supabase database. The
# acknowledgement is intentionally not enough: all three freshness checks must
# pass before the first migration is applied.
...
# Extension-owned objects are part of the Supabase baseline. Any other public
# relation, routine, or standalone type means this target belongs to a user or
# application and is therefore not verifier-safe.
