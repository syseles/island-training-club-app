...
verifier="$repo_root/supabase/tests/verify_giving_campaigns.sh"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/itc-giving-safety.XXXXXX")"
...
case "$query" in
  ...
  *)
    printf 'unexpected mock psql query: %s\n' "$query" >&2
    exit 99 ;;
esac
MOCK
...
run_case() {
  ...
  output="$(ITC_SAFETY_TEST_SCENARIO="$scenario" \
    ITC_GIVING_PSQL_BIN="$tmp_dir/psql" \
    ITC_GIVING_TEST_DATABASE_URL='postgresql://verifier-owned.invalid/postgres' \
...
echo 'Giving campaign verifier safety checks passed.'
